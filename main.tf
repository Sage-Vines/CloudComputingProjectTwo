#this file glues everything together so Terraform knows what to spin up
...
#docker provider is the only "cloud" we touch, keep it simple
provider "docker" {}
...
module "database" {
  ...
  #postgres spins up first so backend has somewhere to stash notes
...
module "backend" {
  ...
  #backend uses stock python image; start.sh installs psycopg and runs app.py
...
module "frontend" {
  ...
  #frontend is just nginx serving static files and proxying /api/* calls
...
output "frontend_url" {
  ...
  value = "http://localhost:${var.frontend_host_port}"  #easy URL to paste into browser
}
