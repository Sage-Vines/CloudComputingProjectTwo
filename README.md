...
- **Backend (Python)** – services/backend/app.py is a framework-free HTTP server. Terraform bind-mounts the folder inside python:3.11-slim, the start.sh script installs psycopg on boot, and the API exposes /api/notes for GET/POST/PUT/DELETE with auto table creation.
