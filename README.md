...
- **Secrets via `.tfvars`** – `database_password` is the only sensitive value and you feed it through `terraform.tfvars` Nothing secret lives within the actual code.
- **Frontend on `localhost:8080`** – module variables pin the port mapping, so the SPA is always reachable on 8080 without extra flags.
